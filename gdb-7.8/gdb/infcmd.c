...
bool hsail_infcmd_is_focus_thread_dispach_thread(void)
{
  ptid_t focus_thread;
  long current_lwp_id;
  focus_thread = thread_get_focus_inferior_ptid();
  current_lwp_id  = focus_thread.lwp;

  if (current_lwp_id == gs_hsail_dispatch_thread_id)
    {
      return true;
    }
  else
...
//Adding step counter to step in a row multiple times one by one
  ...
  if (is_hsail_step())
    {
      /*
       * This change is to work around HSADBG-5 and HSADBG-203,
       * We always use the step function if doing HSAIL debug.
       *
       * This is inefficient since we are sending a *lot more* momentary breakpoints
       * each time and we also cannot step over HSAIL functions. Once this is fixed,
       * use the below function call to set step breakpoints.
       *
...
finish_command (char *arg, int from_tty)
{
  ...
  if (is_hsail_step())
    {
      hsail_set_step_breakpoints(HSAIL_STEP_OUT, 1);
      clear_proceed_status();
      proceed ((CORE_ADDR) -1, 0, 0);
      return;
    }
...
/* Since we cannot change the parameters of kill_command without
 * changing everything in the gdb command handling, we use the
 * below static variables to hold the arguments
 * */
static bool skip_hsa_kill = false;
static bool skip_confirmation_check = false;
static char *kill_command_arg_backup;
static int kill_command_from_tty_backup;
...
kill_command (char *arg, int from_tty)
{
  ...
  kill_command_arg_backup = arg;
  kill_command_from_tty_backup = from_tty;

  bool is_kill_delay_needed = false;
  if (!skip_hsa_kill)
    {
      is_kill_delay_needed = hsail_tdep_kill_all_waves(false);

      /* We reset the flag now, since we'd only want to skip the
       * HSA kill once per kill command
       * */
      skip_hsa_kill = false;
      if (is_kill_delay_needed)
        {
          /* Delay host kill steps and also dont ask if we want to kill
           * the next time through this function */
          skip_confirmation_check = true;
          return;
        }
      /* If no kill delay needed, we just carry on killing the host code */
    }
  else
    {
...
/* Called from the handle hsail event in the FIFO.
 *
 * We have already killed the HSAIL dispatch now.
 * We now call the regular kill command to finish up the remaining
 * host kill functions
 * */
void kill_command_complete(void)
{
  ...
  kill_command (kill_command_arg_backup, kill_command_from_tty_backup);
